...
class Parser {
  ...
  async parseURL(feedUrl, callback, redirectCount=0) {
    let xml = '';
    ...
    let prom = new Promise((resolve, reject) => {
      ...
      let req = get(requestOpts, (res) => {
        ...
        const encoding = utils.getEncodingFromContentType( res.headers['content-type'] );
        const needDecode = utils.needDecodeEncoding( encoding );
        if ( needDecode ) {
          const chunks = [];
          res.on('data', (chunk) => {
            chunks.push(chunk);
          ...
          if(res.headers['content-type'].match("charset")){
            encoding = utils.getEncodingFromContentType(res.headers['content-type']);
            res.setEncoding(encoding);
          }
          ...
          if(res.headers['content-encoding'] == 'gzip' ) {
            ...
            res.on('data', (chunk) => {
              ...
              xml += utils.decode( chunk, encoding );
            });

            res.on('end', () => {
              if(!encoding){
                let xmlEncoding = xml.match(/encoding=[\"|\']([^\"|\']+)[\"|\']/)
  ...
  parseItemRss(xmlItem, itemFields) {
    ...
    if (xmlItem.enclosure) {
      ...
      xmlItem.enclosure.forEach((enclosure) => {
